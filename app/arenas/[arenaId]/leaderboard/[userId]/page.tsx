...
export default async function UserHistoryPage(props: PageProps) {
  const session = await auth()
  if (!session?.user) return redirect("/api/auth/signin")

  const { arenaId, userId } = await props.params
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Avatar className="h-12 w-12">
            ...
            <AvatarFallback>{targetUser.name?.[0] || "U"}</AvatarFallback>
          </Avatar>
          <div className="space-y-1">
            <h1 className="text-3xl font-bold">{targetUser.name}&apos;s History</h1>
            <p className="text-muted-foreground text-sm">Betting history in this arena</p>
          </div>
        </div>
      </div>
              ...
              closedMarkets.map((market) => {
                ...
                return (
                  <TableRow key={market.id}>
                    ...
                    <TableCell className="text-right">{invested}</TableCell>
                    <TableCell className="text-right text-green-600">
                      {payout > 0 ? `+${payout}` : '-'}
                    </TableCell>
                    <TableCell className={`text-right font-bold ${pnl >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {pnl > 0 ? '+' : ''}{pnl}
                    </TableCell>
                  </TableRow>
                )
